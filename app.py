...
app.config['MYSQL_PASSWORD'] = 'Sangamesh@9819'
...
def order_item():
    ...
    if request.method == "POST":
        ...
        Each_price=float(request.form['Each_price'])
        sub_total= QTY * Each_price
        cursor = mysql.connection.cursor()
        res = cursor.execute(''' INSERT INTO order_item(Item_name,QTY,eachprice,sub_total) VALUES(%s ,%s,%s,%s) ''',(Item_name,QTY,Each_price,sub_total))
        mysql.connection.commit()
        cursor.close()
        return render_template('order_item.html', sub_total=sub_total)
    else:
...
def order_table():
    if request.method == "POST":
        ...
        row = cursor.fetchone()
        if exist > 0:
            res = cursor.execute('''INSERT INTO order_table(cust_id, oder_date,oder_time, item_qty)
                                   VALUES(%s, %s, %s, %s)''',
                                 (row[0], order_date, order_time, item_qty))
            mysql.connection.commit()
...
def reservation():
    if request.method == "POST":
        Item_name= request.form['cust_name']
        order_date= request.form['order_date']
        Order_time= request.form['Order_time']
        item_qty=request.form['item_qty'] 
    ...
    return render_template('reservation.html',data_list=data_list,data_list1=data_list1)    
...
def delete(id):
    cursor = mysql.connection.cursor()
    cursor.execute(''' delete from order_table where id = %s''',(id,))
